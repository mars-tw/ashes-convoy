...
[![Release](https://img.shields.io/badge/release-R77-f0b64a)](src/version.js)
...
- **R77 核心戰鬥修補**：增益門核心可被投射物擊破並立即選擇；漏過 gate pair 會清狀態，攻擊改為 anticipation／impact／recovery，低品質不再改敵人數量或 AI 軌跡。
...
- **波次護送玩法**：每波護送車隊深入四種環境，處理屍群、菁英敵人、補給與六類增益門；每 5 波迎戰階段式 Boss。
...
| 手機左下搖桿 | 以可見控制盤移動載具並調整準星，原本全 canvas 拖曳仍保留 |
| 戰鬥中點擊護送車 | 開啟快速升級輪盤，使用既有升級與零件資料 |
...
目前遊戲版本為 **R77**，權威版本值在 [`src/version.js`](src/version.js)。專案程式碼採 MIT License；第三方素材與開發工具的來源、授權及 `image_gen` 產出標註方式請見 [CREDITS.md](CREDITS.md)。
