...
assert.strictEqual(config.APP_VERSION, "R59");
assert.strictEqual(config.CACHE_VERSION, "ashes-convoy-r59-v1");
const indexHtml = fs.readFileSync(path.join(__dirname, "..", "index.html"), "utf8");
assert(indexHtml.includes("manifest.webmanifest?v=R59"), "index.html should version the web manifest for R59");
assert(indexHtml.includes("assets/icons/icon-192.png?v=R59"), "index.html should version the app icon for R59");
...
assert(config.TRAILER_GUNNER && config.TRAILER_GUNNER.enabledDefault === true, "R59 should enable the trailer gunner by default");
...
assert(config.WEAPON_POWERUPS, "R59 should define weapon power-ups");
...
assert.deepStrictEqual(Object.keys(config.ENEMIES).sort(), expectedEnemies.slice().sort(), "enemy roster should match R59 roster");
...
assert.strictEqual(Object.keys(config.TRAILER_ROOM.furniture).length, 15, "R59 should expand the trailer furniture catalog");
...
assert.strictEqual(Object.keys(config.MILESTONES).length, 12, "R59 should define twelve wave milestones");
assert.strictEqual(milestoneRewardTotal, 1080, "R59 milestone rewards should total 1080 parts");
assert.strictEqual(config.MILESTONES.wave_25.target, 25, "R59 wave_25 milestone should target wave 25");
...
assert(config.STORY && Array.isArray(config.STORY.beats), "R59 should define story beats");
assert.strictEqual(config.STORY.beats.length, 12, "R59 should include 12 radio log beats");
...
assert(config.STORY.beats.some((beat) => beat.unlock.type === "bestWave" && beat.unlock.value === 20 && beat.title === "不熄的光"), "R59 story should include the wave 20 ending beat");
