...
assert.strictEqual(config.APP_VERSION, "R63");
assert.strictEqual(config.CACHE_VERSION, "ashes-convoy-r63-v1");
const indexHtml = fs.readFileSync(path.join(__dirname, "..", "index.html"), "utf8");
assert(indexHtml.includes("manifest.webmanifest?v=R63"), "index.html should version the web manifest for R63");
assert(indexHtml.includes("assets/icons/icon-192.png?v=R63"), "index.html should version the app icon for R63");
...
assert(config.TRAILER_GUNNER && config.TRAILER_GUNNER.enabledDefault === true, "R63 should enable the trailer gunner by default");
...
assert(config.WEAPON_POWERUPS, "R63 should define weapon power-ups");
...
assert.deepStrictEqual(Object.keys(config.ENEMIES).sort(), expectedEnemies.slice().sort(), "enemy roster should match R63 roster");
...
assert.strictEqual(Object.keys(config.TRAILER_ROOM.furniture).length, 15, "R63 should expand the trailer furniture catalog");
...
Object.values(config.ENVIRONMENT_EVENTS).forEach((event) => {
  ...
  assert(event.systemLine && Array.isArray(event.barks) && event.barks.length >= 2 && event.completeLine, `${event.id} should include R63 event bark copy`);
...
assert.strictEqual(Object.keys(config.MILESTONES).length, 12, "R63 should define twelve wave milestones");
assert.strictEqual(milestoneRewardTotal, 1080, "R63 milestone rewards should total 1080 parts");
assert.strictEqual(config.MILESTONES.wave_25.target, 25, "R63 wave_25 milestone should target wave 25");
...
assert(config.STORY && Array.isArray(config.STORY.beats), "R63 should define story beats");
assert.strictEqual(config.STORY.beats.length, 12, "R63 should include 12 radio log beats");
...
assert(config.STORY.beats.some((beat) => beat.unlock.type === "bestWave" && beat.unlock.value === 20 && beat.title === "不熄的光"), "R63 story should include the wave 20 ending beat");
