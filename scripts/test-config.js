...
assert.strictEqual(config.APP_VERSION, "R70");
assert.strictEqual(config.CACHE_VERSION, "ashes-convoy-r70-v1");
const indexHtml = fs.readFileSync(path.join(__dirname, "..", "index.html"), "utf8");
assert(indexHtml.includes("manifest.webmanifest?v=R70"), "index.html should version the web manifest for R70");
assert(indexHtml.includes("assets/icons/icon-192.png?v=R70"), "index.html should version the app icon for R70");
...
assert(config.TRAILER_GUNNER && config.TRAILER_GUNNER.enabledDefault === true, "R70 should enable the trailer gunner by default");
assert.strictEqual(config.TRAILER_GUNNER.frames, 2, "R70 should ship idle/fire gunner frames");
...
assert(config.WEAPON_POWERUPS, "R70 should define weapon power-ups");
...
assert.deepStrictEqual(Object.keys(config.ENEMIES).sort(), expectedEnemies.slice().sort(), "enemy roster should match R70 roster");
...
assert.strictEqual(Object.keys(config.TRAILER_ROOM.furniture).length, 15, "R70 should preserve the trailer furniture catalog");
...
Object.values(config.ENVIRONMENT_EVENTS).forEach((event) => {
  ...
  assert(event.systemLine && Array.isArray(event.barks) && event.barks.length >= 2 && event.completeLine, `${event.id} should include R70 event bark copy`);
...
assert.strictEqual(Object.keys(config.MILESTONES).length, 12, "R70 should define twelve wave milestones");
assert.strictEqual(milestoneRewardTotal, 1080, "R70 milestone rewards should total 1080 parts");
assert.strictEqual(config.MILESTONES.wave_25.target, 25, "R70 wave_25 milestone should target wave 25");
...
assert(config.STORY && Array.isArray(config.STORY.beats), "R70 should define story beats");
assert.strictEqual(config.STORY.beats.length, 12, "R70 should include 12 radio log beats");
...
assert(config.STORY.beats.some((beat) => beat.unlock.type === "bestWave" && beat.unlock.value === 20 && beat.title === "不熄的光"), "R70 story should include the wave 20 ending beat");
