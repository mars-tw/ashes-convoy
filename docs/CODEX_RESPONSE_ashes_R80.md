...
## 總稽核審計附註（Claude，2026-07-17，Grok 複審 1 P0＋4 P1）

- Grok P0（p95 上緣 17.6ms 貼門檻＋併發量測）處置：與 farm/td/card 各線同一紀律——本輪 `test:perf` PASS 不作最終出貨判定；六線全收後由總稽核淨機三跑取中位，【回滾條件明定】：任一情境淨機中位 >18ms 即回退 R80（保留 R79.1 合規素材），或降載修復後重審。本輪僅新增靜態視覺資產、不動遊戲迴圈，暫准部署。
- Grok P1-2（R79.1 快取鏈）已由總稽核親驗閉合：三階 atmosphere 的 sw.js precache 與 index.html runtime 引用 hash query 皆與實檔 SHA-8 一致（31cbd67f/115410f5/b58b835b），cache 升 `ashes-convoy-r80-v1` 全量換血。
- Grok P1-5（家具＝新玩法疑慮）查證駁回：家具/強化系統為 R53 引入、R57 加故事層之既有系統，R80 僅換視覺，無違 Wave 3 暫緩令。
- Grok P1-4 採納：來源表「promo runtime」用語需理解為「後製產出物雜湊登記」，promo 四圖確認不入 runtime（basename 掃描 0 命中、解碼記憶體 0 計入）；下輪 manifest 用語應區分 runtime/deliverable。
- C2PA 6/6（R79.1×1＋R80×5）由總稽核親驗 marker。
