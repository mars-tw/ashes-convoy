...
- 避難所基地場景：
  - meta 主畫面由平淡車庫改為第一人稱列車車廂內部避難所。
  - 場景包含床與熟睡少女、細膩毛毯、串燈、補給層架、暖黃桌燈、收音機、便利貼清單、泰迪熊、植物或太陽能設備。
  - 車廂盡頭窗外必須有冷藍霧氣、雪或灰塵，以及逼近的喪屍剪影動畫。
  - 美術目標是暖橘室內安全感與冷藍窗外危險感的強對比。
  - 使用 `src/shelter-scene.js` 合成 full-bleed Canvas 場景，遊戲端在 `SHELTER_HOTSPOTS` 上疊 DOM 按鈕。
  - 出勤、升級、切換載具、補給或系列入口必須從場景熱區進入。
- 像素動畫完整化：
  - Boss 階段變化。
  - 載具受損狀態。
  - 爆炸、命中、護盾、酸霧、電弧。
...
- 避難所場景在 390 x 844 手機直式下第一眼可讀出：床與熟睡少女、暖光、補給、窗、窗外喪屍剪影。
- 場景氛圍符合「室內暖橘安全 vs 窗外冷藍危險」，不是單色背景或普通車庫面板。
- `drawShelterScene(ctx, opts)` 繪出的場景 canvas 非空，且主要元素位於可見安全區。
- `SHELTER_HOTSPOTS` 能對齊 DOM 按鈕；出勤入口可點擊並進入射擊局。
- 390 x 844、820 x 1180、1280 x 900 三平台自適應，DOM 按鈕不偏離熱區且不遮擋熟睡少女臉部或窗外喪屍。
- 窗外喪屍剪影有低頻 walk 或逼近動畫，不能只是靜態裝飾。
- `reducedFlash` 開啟時，串燈、收音機與燈泡閃爍停止或降為穩定亮度。
- Playwright E2E 驗證避難所 canvas 非空、出勤入口可點、升級或載具入口至少一個 hotspot 可點。
